...
static std::string tis620_to_utf8(const char* s) {
    ...
    for (const unsigned char* p = (const unsigned char*)s; *p; ++p) {
        ...
        if (c < 0x80) {
            ...
            out.push_back('?'); // ????��???? TIS620 range
        }
    }
    return out;
}

// escape ��?����?? JSON + ???? TIS620 ???? UTF8 ??��?
	...
	if (md) {
		...
		if (md->spawn->state.boss) {
			...
			std::snprintf(announce_msg, sizeof(announce_msg),
				"[MVP Spawn]: %s ��鹤׹�վ�ա���駷��Ἱ��� %s �պ仡ӨѴ�ǡ�ѹ��!",
				md->name, mapname);
    		...
    		job.desc  = std::string(md->name) + " ���Դ��鹷��Ἱ��� " + map_mapid2mapname(md->spawn->m);
	...
	{ // Item Drop
		...
		for( const std::shared_ptr<s_mob_drop>& entry : md->db->dropitem ){
			...
			strftime(timestr, sizeof(timestr), "%Y-%m-%d %H:%M:%S", &t);
    			...
    			job.url   = "https://discord.com/api/webhooks/1421372464189149266/vzWJad4HTuxVzZQ4VnBPZgNCILNYrOo8DpHQxT6Mk3zAnUCTgRdXZUqOgifhFV74njL3"; // webhook �ͧ�س
    			job.user  = "BeastRO Announce";
    			job.title = "";
    			...
    			snprintf(desc, sizeof(desc),
    			    ...
    			    now_ymdhms_gmt7().c_str()   // helper �׹������
    			);
    			job.desc  = desc;
    			job.color = 0xFF0000; // ᴧ
			
				// �ٻ�����: �������ٻ�ҡ Divine-Pride, ��� � �� RMS
				char img_url[256];
				if (it && it->type == IT_CARD) {
				    // ���� (�� 4302) ? https://static.divine-pride.net/images/items/cards/4302.png
