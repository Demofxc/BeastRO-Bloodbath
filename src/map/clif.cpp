...
void clif_name( struct block_list* src, struct block_list *bl, send_target target ){
	...
	switch( bl->type ){
		...
		case BL_PET: {
			...
			struct pet_data *pd = BL_CAST(BL_PET, bl);

			char extra_name[50];
			char owner_name[50]; 

			map_session_data *ssd = pd->master;

			char refine_name[10] = { 0 };
			char grade_name[10] = { 0 };
			std::string new_name = "";
			std::string ownertext = "Ã Â¨Ã©Ã’Â¢ÃÿÂ§ÃÿÃ‘ÂµÃÿÃ¬Ã Ã…Ã•Ã©ÃÿÂ§";

			if(pd->refine)
				sprintf(refine_name, "%d", pd->refine);

			if(pd->enchantgrade)
				sprintf(grade_name, "%s", pet_grade_text(pd->enchantgrade).c_str());

			if(pd->refine){
				new_name += "(+";
				new_name += refine_name;
				new_name += ")";
			}
			...
			if (ssd) {
				...
				sprintf(owner_name, "%s Ã¤ÃÿÃ¨ÃÿÃÿÃ©Â¨Ã‘Â¡", ownertext.c_str());
			}

			bool use_new_name = false;

			if(strlen(new_name.c_str())){
				use_new_name = true;
				sprintf(extra_name, "%s %s", pd->pet.name , new_name.c_str());
			}

			size_t len = strlen(extra_name);
			if(len > NAME_LENGTH){
				memcpy(extra_name, extra_name + len - NAME_LENGTH, NAME_LENGTH);
				extra_name[NAME_LENGTH] = '\0';
			}

			if(use_new_name) {
				safestrncpy( packet.name, extra_name, NAME_LENGTH);
				safestrncpy( packet.guild_name, ownertext.c_str(), NAME_LENGTH );
				safestrncpy( packet.position_name, owner_name, NAME_LENGTH );
			}
			else {
				safestrncpy( packet.name, pd->pet.name, NAME_LENGTH );
				safestrncpy( packet.guild_name, ownertext.c_str(), NAME_LENGTH );
				safestrncpy( packet.position_name, owner_name, NAME_LENGTH );
			}
		...
		case BL_MOB: {
			...
			if( md->guardian_data && md->guardian_data->guild_id ){
				...
				int ele_group = -1;
				char ele_name[8] = {0};
				char output[100]     = {0};   // ¾ÍÊÓËÃÑº "Undead" ÏÅÏ + à¼×èÍ
				char name_table[100] = {0};   // ¢éÍ¤ÇÒÁª×èÍ + Lv + ¢¹Ò´
				char size_name[16]  = {0};   // "Medium"/"Large" ÏÅÏ
				char race_name[16]  = {0};   // "Medium"/"Large" ÏÅÏ
				...
				if(battle_config.mob_ele_view)
				{
					...
					switch (md->status.size) {
						case 0: strcpy(size_name, "Small");break;
						case 1: strcpy(size_name, "Medium");break;
						case 2: strcpy(size_name, "Large");break;
						default: break;
					}
					...
					switch (md->status.race) {
						...
						case 7: strcpy(race_name, "Demihuman");break;
					...
					{
						if(ele_group > -1)
							ud->group_id = ele_group;  // ãªé ud â´ÂµÃ§
						
						if(strlen(ele_name))
						{
							//¡ÓË¹´ª×èÍ
							safesnprintf(name_table,sizeof(name_table),"Lv.%d %s ( %s )",md->level, md->name, size_name);
							safestrncpy(ud->title, name_table, NAME_LENGTH);

							safesnprintf(output,sizeof(output),"%s Lv.%d [ %s ]",ele_name, md->status.ele_lv, race_name);
							safestrncpy(md->name, output, NAME_LENGTH);
